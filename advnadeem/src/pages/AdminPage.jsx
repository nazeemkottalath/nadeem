    ...
    <div>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 7, padding: '1.75rem' }}>
        ...
        <Field label="Cover Image (optional)">
          <div style={{ background: C.offWhite, border: `1px solid ${C.border}`, borderRadius: 6, overflow: 'hidden' }}>
            {/* Tab switcher */}
            <div style={{ display: 'flex', borderBottom: `1px solid ${C.border}` }}>
              {[['url','Paste URL'], ['upload','Upload File']].map(([id, label]) => (
                <button key={id} onClick={() => setImgTab(id)} style={{
                  flex: 1, background: imgTab === id ? C.white : 'transparent',
                  border: 'none', borderBottom: imgTab === id ? `2px solid ${C.navy}` : '2px solid transparent',
                  padding: '9px', cursor: 'pointer', fontFamily: "'DM Mono', monospace",
                  fontSize: '0.72rem', letterSpacing: '0.08em', color: imgTab === id ? C.navy : C.textMuted,
                }}>{label}</button>
              ))}
            </div>
            ...
            <div style={{ padding: '1rem' }}>
              ...
              {imgTab === 'upload' && (
                <div>
                  <label style={{
                    ...
                    background: C.white, border: `1px dashed ${C.borderDark}`,
                    borderRadius: 6, padding: '1rem', cursor: 'pointer',
                  }}>
                    <span style={{ fontSize: '1.5rem' }}>📁</span>
                    <div>
                      <div style={{ color: C.navy, fontSize: '0.875rem', fontWeight: 600 }}>
                        {uploading ? 'Uploading…' : 'Choose image file'}
                      </div>
                      <div style={{ color: C.textFaint, fontFamily: "'DM Mono', monospace", fontSize: '0.68rem', marginTop: '0.2rem' }}>
                  ...
                  {uploadErr && <p style={{ color: C.red, fontSize: '0.8rem', marginTop: '0.5rem' }}>{uploadErr}</p>}
                </div>
              )}
              ...
              {p.cover_image && (
                <div style={{ marginTop: '0.75rem', position: 'relative', display: 'inline-block' }}>
                  <img src={p.cover_image} alt="Cover preview"
                    style={{ width: '100%', maxHeight: 200, objectFit: 'cover', borderRadius: 5, display: 'block' }}
                    onError={e => e.target.style.display = 'none'}
                  />
                  <button onClick={() => set('cover_image', '')} style={{
        ...
        <button onClick={() => onSave(p)} disabled={saving || uploading}
          style={{ background: (saving || uploading) ? C.textFaint : C.navy, color: C.white, border: 'none', padding: '11px 26px', borderRadius: 5, cursor: (saving || uploading) ? 'not-allowed' : 'pointer', fontFamily: "'DM Mono', monospace", fontWeight: 700, fontSize: '0.85rem', letterSpacing: '0.08em' }}>
